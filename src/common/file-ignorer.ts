...
export class FileIgnorer implements Ignorer<string>, AsyncDisposable {
  ...
  readonly #ignores: Map<string, Ignore> = new Map();
  ...
  public constructor(dir: string, { logger = console, watch = true }: IgnorerOptions = {}) {
    ...
    if (watch) {
      ...
      this.#watcher.on('add', () => this.scanImports());
      this.#watcher.on('change', () => this.scanImports());
      this.#watcher.on('unlink', () => this.scanImports());
    }
  }

  private scanImports(): void {
    this.logger.debug(`scanning .gitignore files`);
    this.#ignores.clear();
